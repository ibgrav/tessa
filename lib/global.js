...
export const storageFetch = async ({ base, prefix, delimiter, type, sort, desc }) => {
    const url = `${base ? base : `${location.origin}/`}api/storage?prefix=${prefix}${addQuery({ delimiter })}${addQuery({ type })}${addQuery({ sort })}${addQuery({ desc })}`;
