...
export const storageFetch = async ({ prefix, delimiter, type, sort, desc }) => {
    const url = `http://localhost:3000/api/storage?prefix=${prefix}${addQuery({ delimiter })}${addQuery({ type })}${addQuery({ sort })}${addQuery({ desc })}`;
